...
function jsHarness(slug, code, tc) {
...
function pyHarness(slug, code, tc) {
...
function cppHarness(slug, code, tc) {
    ...
    const calls = tc.map(x => {
        ...
        if (slug === "reverse-string")
            return `cout << ${fn}("${String(a).replace(/"/g, '\\"')}") << "\\n";`;
        return `cout << (${fn}("${String(a)}") ? "true" : "false") << "\\n";`;
    }).join("\n");
    return `#include <bits/stdc++.h>\nusing namespace std;\n${code}\nint main(){${calls}}\n`;
}
function javaHarness(slug, code, tc) {
    const fn = slug === "two-sum" ? "twoSum" : slug === "reverse-string" ? "reverseString" : slug === "valid-parentheses" ? "isValid" : "search";
    const calls = tc.map(x => {
        const [a, b] = x.input;
        if (slug === "two-sum")
            return `System.out.println(Arrays.toString(s.${fn}(new int[]{${a.join(",")}}, ${b})));`;
        if (slug === "binary-search")
            return `System.out.println(s.${fn}(new int[]{${a.join(",")}}, ${b}));`;
        if (slug === "reverse-string")
            return `System.out.println(s.${fn}("${String(a).replace(/"/g, '\\"')}"));`;
...
async function runSubmission(id) {
    ...
    if (!sub)
        return;
    const tests = Array.isArray(sub.problem.testCases) ? sub.problem.testCases : [];
    const dir = await mkdtemp(join(tmpdir(), "matiks-"));
    const started = Date.now();
    await prisma.submission.update({ where: { id }, data: { status: "RUNNING" } });
    try {
        let file = "", args = [], command = "";
        if (sub.language === "javascript") {
            file = join(dir, "main.js");
            await writeFile(file, jsHarness(sub.problem.slug, sub.code, tests));
            command = "node";
            args = [file];
        }
        else if (sub.language === "python") {
            file = join(dir, "main.py");
            await writeFile(file, pyHarness(sub.problem.slug, sub.code, tests));
            command = "python3";
            args = [file];
        }
        else if (sub.language === "cpp") {
            file = join(dir, "main.cpp");
            await writeFile(file, cppHarness(sub.problem.slug, sub.code, tests));
            command = "g++";
            args = [file, "-std=c++17", "-O2", "-o", join(dir, "main")];
        }
        else {
            file = join(dir, "Main.java");
            await writeFile(file, javaHarness(sub.problem.slug, sub.code, tests));
            command = "javac";
            args = [file];
        }
        ...
        for (let i = 0; i < Math.min(lines.length, normalized.length); i++) {
            ...
            try {
                ...
                actual = lines[i].replace(/^['"]|['"]$/g, "");
            }
            if (equal(actual, tests[i].output))
                passed++;
        }
