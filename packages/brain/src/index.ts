...
export function updateBrain(
  input: BrainInput,
  result: { correct: boolean; responseMs: number; expectedMs: number; topic: string }
): BrainOutput {
  const accuracy = result.correct ? 1 : 0;
  const speed = clamp(
    1 - result.responseMs / Math.max(result.expectedMs, 1) / 2,
    0,
    1
  );
  const alpha = 0.18;
  ...
  const consistency =
    recentResults.reduce((a, b) => a + b, 0) / Math.max(recentResults.length, 1);

  const delta = (accuracy - 0.5) * 10 + (speed - 0.5) * 6;
  const volatility = Math.abs(delta) * 0.12 + input.volatility * 0.88;
  const edge = clamp(
    Math.round(
      input.currentEdge + delta * (1 - Math.min(volatility / 10, 0.65))
    ),
    1,
    200
  );
  ...
  topicScores[result.topic] = Math.round(
    (topicScores[result.topic] ?? 50) * 0.82 + accuracy * 100 * 0.18
  );
  ...
  const confidence = clamp(
    input.confidence * 0.85 + Math.min(recentResults.length / 20, 1) * 0.15,
    0,
    1
  );
  ...
  return {
    ...
    recommendedRange,
    confidenceBand:
      confidence < 0.35 ? "low" : confidence < 0.7 ? "medium" : "high",
    confidence,
  };
}

export function rankDistance(a: number, b: number) {
  return Math.abs(a - b);
}
